...
import { Calendar, Clock, Edit, Eye, MoreHorizontal, Trash2, User, Users } from "lucide-react";
...
export function SessionCard({ session, variant = "compact", showDate = true, onEdit, onRefresh }: SessionCardProps) {
  const updateSessionMutation = useUpdateSession();
  const deleteSessionMutation = useDeleteSession();

  const handleStatusUpdate = (sessionId: string, newStatus: "SCHEDULED" | "CANCELLED" | "COMPLETED") => {
    updateSessionMutation.mutate({ sessionId, data: { status: newStatus } }, { onSuccess: () => onRefresh?.() });
  };
            ...
            <div className="min-w-0 flex-1 space-y-2">
              ...
              <div className="flex items-center justify-between">
                <div className="flex min-w-0 flex-1 items-center gap-2">
                  ...
                  />
                  <h3 className="truncate text-sm font-semibold">{session.item.name}</h3>
                </div>
                <Badge
                  variant="secondary"
              ...
              {/* Notes */}
              {session.notes && (
                <div className="muted-foreground bg-muted/30 border-muted rounded border-l-2 p-2 text-xs italic">
                  {session.notes}
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-1 opacity-0 transition-opacity group-hover:opacity-100">
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  ...
                  <DropdownMenuItem onClick={() => console.log("View session details")} className="text-xs">
                    <Eye className="mr-2 h-3.5 w-3.5" />
                    View Details
                  </DropdownMenuItem>

                  <DropdownMenuItem onClick={() => onEdit?.(session)} className="text-xs">
                  ...
                  {session.status !== "SCHEDULED" && (
                    <DropdownMenuItem onClick={() => handleStatusUpdate(session.id, "SCHEDULED")} className="text-xs">
                      Mark as Scheduled
                    </DropdownMenuItem>
                  )}

                  {session.status !== "CANCELLED" && (
                    <DropdownMenuItem onClick={() => handleStatusUpdate(session.id, "CANCELLED")} className="text-xs">
                      Cancel Session
                    </DropdownMenuItem>
                  )}

                  {session.status !== "COMPLETED" && (
                    <DropdownMenuItem onClick={() => handleStatusUpdate(session.id, "COMPLETED")} className="text-xs">
                      Mark as Completed
                    </DropdownMenuItem>
                  )}
                  ...
                  <DropdownMenuItem
                    ...
                    disabled={(session._count?.bookings || 0) > 0}
                  >
  ...
  // Detailed variant for day details dialog
  return (
    <Card
      className="group border-l-4 transition-all duration-200 hover:shadow-md"
      style={{ borderLeftColor: session.item.color || SESSION_STATUS_COLORS[session.status] }}
