...
export function TabTriggers({ memberDetails, memberRole, scheduledSessionCount }: TabTriggersProps) {
    ...
    return (
      <TabsList className="**:data-[slot=badge]:bg-muted-foreground/30 w-fit **:data-[slot=badge]:size-5 **:data-[slot=badge]:rounded-full **:data-[slot=badge]:px-1">
        ...
        <TabsTrigger value="sessions">
          Sessions {sessionCount > 0 ? <StatusBadge variant="secondary">{sessionCount}</StatusBadge> : null}
        </TabsTrigger>
      </TabsList>
    );
  }
